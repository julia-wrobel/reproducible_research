...
<iframe class="responsive-iframe" style="position: absolute; top: 0; left: 0; right: 0; width: 100%; height: auto; aspect-ratio: 560 / 314" title="Motivation notes" src="slides/projectorg_slides.pdf"></iframe>
</div>
...
To be able to access these parameters, I added the following R code chunk to `final_report.Rmd`:


```{r set_params, eval = FALSE}
state = tolower(params$state)
download_raw_data = params$download_raw_data
```

To be able to filter by state, we will need to edit the file `02_data_cleaning.R` to filter by this `state` variable, and make it into a function that reads in `state` as a parameter.

```{r, eval = FALSE}
# define function that takes in state as a parameter and cleans the data
clean_data = function(state){
